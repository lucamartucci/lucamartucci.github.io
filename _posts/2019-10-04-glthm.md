...
Note: we use $$x\sim \{0, 1\}^n$$ to de note uniform distribution over $$\{0, 1\}^n$$
...
Given (as in the Goldreich-Levin theorem) $Pr_{x, r} [A(f(x),r)\ =\ \langle x ,r\rangle] \geq \frac{1}{2} + \epsilon$, we first show that there is a certain fraction of $x$,  $Pr_r[A(f(x), r) = \langle x, r \rangle] \geq \frac{1}{2} + \frac{1}{2}\epsilon$ holds:  

$Pr_{x, r} [A(f(x),r)\ =\ \langle x ,r\rangle]$ can also be viewed as $\sum_x Pr(x)Pr_r[A(f(x),r)\ =\ \langle x ,r\rangle]$,  which is equal to the expectation $E_x[\ Pr_r[A(f(x),r)\ =\ \langle x ,r\rangle]\ ]$ ( so we view $Pr_r[A(f(x),r)\ =\ \langle x ,r\rangle]$ as a random variable of $x$). Now we have the expectation is greater than $1/2 + \epsilon$, by the Markov ineuality (Theorem 1 in background) we have:  
