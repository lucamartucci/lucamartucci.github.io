...
**Theorem 3 (Cherbyshev Inequality)**  
Suppose $X= X_1 + \cdots + X_k$ where $X_1, \ldots ,X_k$ are $0, 1$ pairwise independent variables and $t>0$, then  

$$Pr[|X-E[X]| \geq t] \leq \frac{Var(X_1) + \cdots + Var(X_k)}{t^2}$$

In particular, if for each $X_i$, $Pr[X_i = 1] \geq 1/2 + \epsilon$, then $Var(X_i)<\frac{1}{4}$, therefore 

$$Pr[|X-E[X]| \geq t] \leq \frac{Var(X_1) + \cdots + Var(X_k)}{t^2}\leq \frac{k}{4t^2}$$

**Proof** 

Let $S$ be the set of $x$ such that $|X(x)-E[X]| \geq t$
...
this proof can be generalized to prove $Var[X_1 + \cdots + X_k]= Var[X_1] + \cdots + Var[X_k]$
...
* randomly sample $$r_1, \ldots, r_k \in \{0, 1\}^n$$ where $k =  \frac{4\log n}{\epsilon^2}$
* for $$b_1, \ldots, b_k \in \{0, 1\}$$ (each $b_j$ corresponds to a possible value of $\langle x, r_j \rangle$) :  
    * for $i= 1, 2, \ldots, n$:
        * compute $H(r) - b_j$  
        * compute $$x_i = H^\prime_{r_1 \ldots r_k}(e_i) = Majority\{H(r_j + e_i) - b_j: j = 1, 2, \ldots , 4\log n/ \epsilon^2 \}$$
    * add $x$ to list $L$
* return $L$

The correct $x$ should be in the list $L$ with probability $1- 1/n$.  
...
* GL algorithm:
* Randomly sample $$r_1, \ldots, r_k \in \{0, 1\}^n$$ where $k =  O(\log \frac{4\log n}{\epsilon^2})$
...
* for all $$b_1 \ldots, b_k \in \{0, 1\}$$:  
    ...
    * for each $r_S$:  
        * compute $H(r_S+e_i) - b_S$
        
    * compute $$x_i = H^\prime_{r_1 \ldots r_k}(e_i) = Majority\{H(r_j + e_i) - b_S:\ S\subseteq \{r_1, \ldots, r_k \} \}$$
    * add $x$ to list $L$
* return $L$

Now we analyze this GL algorithm
