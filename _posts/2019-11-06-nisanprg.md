...
$$\lvert\lvert M^r[G(x)] - M^r[x_1 \ldots x_r]\rvert\rvert \leq \epsilon$$  
...
$$G_{h_1,\ldots ,h_{\log r}} = x,h_1(x), h_2(x), h_2(h_1(x)), \ldots, h_{\log r}(x),\ldots h_{\log r}(h_{\log r-1}(\ldots h_1(x)))$$  

**Theorem 3.4** (Nisan) For $d, r$ and $m<d$, there is an $(m,r)$-generator $$G:\{0,1\}^m\rightarrow \{0,1\}^{mr}$$ that is $1/d$-pseudorandom for $M^r$. Moreover, the generator takes a seed of length $O(m+\log r)$ and can be computed in space $O(m+\log r)$.  
