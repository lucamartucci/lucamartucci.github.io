...
**Definition 2.4** (Pairwise Independent Hashing Family) We call a family of hashing functions $$H:\{0,1\}^m\rightarrow \{0,1\}^n$$ is pairwise independent if for any $$x_1, x_2 \in \{0,1\}^m$$ and $x_1 \neq x_2$, $$y_1, y_2\in \{0,1\}^n$$ , when $$h:\{0,1\}^m\rightarrow \{0,1\}^n$$ is chosen randomly from $H$, then  
...
**Theorem 3.4** (Nisan) For $d, m$ and $r<d$, there is an $(m,r)$-generator $$G:\{0,1\}^m\rightarrow \{0,1\}^{mr}$$ that is $1/d$-pseudorandom for $M^r$. Moreover, the generator takes a seed of length $O(m\log r)$ and can computes its output in space $O(m+\log r)$.  
