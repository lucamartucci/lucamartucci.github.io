...
Recall our goal is to construct a pseudorandom generator for the above ROBPs. This is equivalent to we want that the final matrix $M_{prg}^r$ obtained by feeding the ROBP with pseudorandom bits is close to the final matrix $M^r$ obtained by feeding with uniformly random bits.
